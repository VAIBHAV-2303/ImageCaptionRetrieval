...
with open('data/train/image_features.pkl', 'rb') as f:
	ims = pickle.load(f)
	ims = ims/torch.norm(ims, dim=1, p=2).reshape(-1, 1)
with open('data/train/captions.pkl', 'rb') as f:
	caps = pickle.load(f)
...
with open('data/val/captions.pkl', 'rb') as f:
...
for epoch in range(max_epochs):
	...
	for batch_index in range(0, ims.shape[0], batch_size):
		...
		curr_caps = caps[batch_index:batch_index+batch_size]
	...
	for i in range(len(val_caps)):
		...
		r.append(np.where(ranks==i)[0][0] + 1)
